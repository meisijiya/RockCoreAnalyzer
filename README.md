...
**v1.2.0 已冻结稳定。** 下列功能欢迎通过 Pull Request 实现，详细贡献流程见 [CONTRIBUTING.md](CONTRIBUTING.md)。

### 🟢 High Priority (推荐新手)

| 功能 | 难度 | 描述 | 涉及文件 |
|------|------|------|---------|
| **跨模块综合报告** | ⭐⭐ | 同一岩心同时跑孔洞+裂缝+粒度,生成一份综合 PDF | `core/report_exporter.py` |
| **多图像批处理 UI** | ⭐⭐ | GUI 选目录批量分析,进度条 | `gui/main_window.py` |
| **图像直方图** | ⭐ | Step 8 加灰度直方图面板 | `gui/pore_module.py` |
| **导出报告模板定制** | ⭐⭐ | 允许用户选报告 layout (简洁/详细/科研) | `core/report_exporter.py` |
| **CSV 数据导入分析** | ⭐ | 从 CSV 读已有孔洞数据,直接出报告 | 新模块 |

### 🟡 Medium Priority

| 功能 | 难度 | 描述 |
|------|------|------|
| **多尺度粒度分析** | ⭐⭐⭐ | 大/中/小颗粒分别统计（分级 watershed） |
| **颗粒方向性分析** | ⭐⭐ | 各向异性玫瑰图（长轴方向分布） |
| **沉积相自动判别** | ⭐⭐⭐ | 基于 Folk & Ward 粒度参数（Mz/σ1/Sk/KG） |
| **图像配准** | ⭐⭐⭐ | 多张岩心图像对齐拼图 |
| **自定义分类阈值 UI** | ⭐⭐ | 让用户自由定义大/中/小洞边界 |

### 🔴 Low Priority (研究性质)

| 功能 | 难度 | 描述 |
|------|------|------|
| **深度学习辅助** | ⭐⭐⭐⭐⭐ | U-Net 端到端分割（需 GPU + 训练数据） |
| **三维可视化** | ⭐⭐⭐⭐ | 同一岩心多角度 3D 重建 |
| **Web 版 (FastAPI)** | ⭐⭐⭐⭐ | 浏览器访问 |
| **云端协作** | ⭐⭐⭐⭐⭐ | 多用户协作 + 远程分析 |
| **数据库存储** | ⭐⭐⭐ | SQLite/PostgreSQL 存历史分析 |

### 🐛 已知小问题 (也欢迎 PR)

- [ ] 大图 (>4K) 加载时偶尔 OOM
- [ ] 触摸板手势缩放不流畅（仅 Windows）
- [ ] 报告文件名含中文时偶发编码问题
- [ ] Step 5 多次重提取后偶尔 OTSU 内存泄漏

---

## 🤝 如何贡献

1. **Fork 仓库** → 创建 `feature/xxx` 分支
2. **开发 + 测试** → 必须保持 142 个测试 100% 通过
3. **更新文档** → CHANGELOG.md / 相关 README
4. **提 PR** → 描述清楚改动 + 关联 Issue
...
- **`docs/CODE_GUIDE.md`** - ⭐ **零基础代码讲解手册**（含 30 个验收 Q&A + 紧急救援）
- **`CONTRIBUTING.md`** - 贡献指南 + 待实现功能详细列表
- **`岩心孔洞软件.pdf`** - 项目原始需求文档
...
**直接看 `docs/CODE_GUIDE.md`！**

- 30 分钟演示脚本
- 30 个 Q&A（覆盖常见质疑）
- 10 个常见运行错误排查
- 零基础 Python/OpenCV/PyQt5 速通
- 核心算法（OTSU/Hough/分水岭）代码导读

---

## 📜 版本历史

- **v1.2.0** (2026-06-23) ⭐ **当前稳定版** (已冻结)
- **v1.1.5** (2026-06-23) 拖拽式画笔
- **v1.1.0** (2026-06-23) 裂缝模块
- **v1.0.0** (2026-06-23) 孔洞核心功能发布

> **v1.2.0 已冻结** — 仅修关键 bug。新功能通过 PR 到 `master` 分支。
